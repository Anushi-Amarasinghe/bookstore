...
async function sendWelcomeEmail(email) {
  try {
    ...
    await transporter.sendMail({
      from: 'bookshop281@gmail.com',
      to: email,
      subject: 'Welcome to the bookshop',
...
app.get('/contact', (req, res, next)=>{
  res.render('contact', {title : 'Contact', stylesheet : '/resources/styles/contact.css'});
})
...
app.post('/contact', (req, res) => {
  ...
  db.run(
    "INSERT INTO UserMessages (name, email, message, submitted_at) VALUES (?, ?, ?, ?)",
...
app.get('/about', (req, res) => {
  // Assuming you have user session or authentication logic to fetch the username
  const username = req.session ? req.session.username : null; // Adjust as per your session or login setup
  res.render('aboutus', { username: username }); // Pass the username to the view
});
